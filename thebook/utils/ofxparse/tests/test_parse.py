...
from thebook.utils.ofxparse import (
    Account,
    AccountType,
    OfxParser,
    Statement,
...
from thebook.utils.ofxparse.ofxparse import (
    OfxFile,
    OfxParserException,
    OfxPreprocessedFile,
    soup_maker,
...
class TestParse(TestCase):
    ...
    def testEverything(self):
        ...
        self.assertEqual(
            datetime(2009, 5, 23, 12, 20, 17), ofx.account.statement.balance_date
        )
        # Todo: support values in decimal or int form.
        # self.assertEqual('15',
        # ofx.bank_account.statement.balance_in_pennies)
        self.assertEqual(Decimal("682.34"), ofx.account.statement.available_balance)
        self.assertEqual(
            datetime(2009, 5, 23, 12, 20, 17),
            ofx.account.statement.available_balance_date,
        )
        self.assertEqual(datetime(2009, 4, 1), ofx.account.statement.start_date)
        self.assertEqual(
    ...
    def test_file_linebreak_before_headers_with_non_ascii_content(self):
        with open_file(
            "linebreak-before-headers-with-non-ascii-content.ofx"
        ) as ofx_file:
...
class TestStringToDate(TestCase):
    ...
    def test_parses_correct_time(self):
        ...
        self.assertEqual(
            OfxParser.parseOfxDateTime("19881201"), datetime(1988, 12, 1, 0, 0)
        )
        self.assertEqual(
            OfxParser.parseOfxDateTime("19881201230100"), datetime(1988, 12, 1, 23, 1)
        )
        self.assertEqual(
            OfxParser.parseOfxDateTime("20120229230100"), datetime(2012, 2, 29, 23, 1)
        )

    def test_parses_time_offset(self):
        """Test that we handle GMT offset"""
        self.assertEqual(
            OfxParser.parseOfxDateTime("20001201120000 [0:GMT]"),
            datetime(2000, 12, 1, 12, 0),
        )
        self.assertEqual(
            OfxParser.parseOfxDateTime("19991201120000 [1:ITT]"),
 ...
 <STMTRS>
  ...
  <AVAILBAL>
   ...
   <DTASOF>20090523122017
        ...
        self.assertEqual(datetime(2009, 5, 23, 12, 20, 17), statement.end_date)
        ...
        self.assertEqual(
            datetime(2009, 5, 23, 12, 20, 17), statement.available_balance_date
        )
...
"""
        ...
        self.assertEqual("pos", transaction.type)
        self.assertEqual(
            datetime(2009, 4, 1, 12, 20, 17) - timedelta(hours=-5), transaction.date
        )
...
class TestVanguardInvestmentStatement(TestCase):
    def testForUnclosedTags(self):
        ...
        self.assertEqual(len(ofx.account.statement.transactions), 1)
        self.assertEqual(
            ofx.account.statement.transactions[0].id, "01234567890.0123.07152011.0"
        )
        self.assertEqual(
            ofx.account.statement.transactions[0].tradeDate, datetime(2011, 7, 15, 21)
        )
        self.assertEqual(
            ofx.account.statement.transactions[0].settleDate, datetime(2011, 7, 15, 21)
        )
        self.assertTrue(hasattr(ofx.account.statement, "positions"))
        self.assertEqual(len(ofx.account.statement.positions), 2)
        self.assertEqual(ofx.account.statement.positions[0].units, Decimal("102.0"))
...
class TestVanguard401kStatement(TestCase):
    def testReadTransfer(self):
        ...
        self.assertEqual(
            ofx.account.statement.transactions[-1].id, "1234567890123456795AAA"
        )
        self.assertEqual("transfer", ofx.account.statement.transactions[-1].type)
        self.assertEqual(ofx.account.statement.transactions[-1].inv401ksource, "MATCH")
...
class TestTiaaCrefStatement(TestCase):
    ...
    def testReadPositions(self):
        ...
        self.assertEqual(len(ofx.account.statement.positions), len(expected_positions))
...
class TestFidelityInvestmentStatement(TestCase):
    def testForUnclosedTags(self):
        ...
        self.assertEqual(ofx.account.statement.positions[0].units, Decimal("128.0"))
        self.assertEqual(
    ...
    def testBalanceList(self):
        ...
        self.assertEqual(
            ofx.account.statement.balance_list[0].description,
            "The net market value of all long and short positions in the account",
        )
        self.assertEqual(
            ofx.account.statement.balance_list[0].value, Decimal("32993.79")
        )
...
class TestFidelitySavingsStatement(TestCase):
    def testSTMTTRNInInvestmentBank(self):
        ...
        self.assertEqual(datetime(2012, 7, 20, 0, 0, 0) - timedelta(hours=-4), tx.date)
        ...
        self.assertEqual(datetime(2012, 7, 27, 0, 0, 0) - timedelta(hours=-4), tx.date)
    ...
    * STMTRS.BANKTRANLIST.STMTTRN.CHECKNUM (optional)
    ...
    def testMissingTransactionHeader(self):
        ...
        self.assertTrue(
            ...
            .startswith("Currency definition was empty for <stmtrs><curdef></curdef>")
        )
...
class TestSuncorpBankStatement(TestCase):
    def testCDATATransactions(self):
        ...
        self.assertEqual(
            transaction.memo, "EFTPOS WDL HANDYWAY ALDI STORE   GEELONG WEST VICAU"
        )
